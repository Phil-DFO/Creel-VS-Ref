...
#Bute & Toba
Bute_Toba_Catch <- ests %>% filter(creelsub %in% Bute_Toba, month %in% Bute_Toba_Months, species == "Chinook") %>%
...
est3 <- est2 %>% 
  filter(!(creelsub %in% c("16N", "16P", "16J", "16B", "16K", "17J", "17K", "15E", "15.6", "13L", "13.21") 
           & month == 7 & daterange %in% c("1531", "0131"))) #the part of the gulf MSF in 17 is ther only aras that had a refernce fishery when the MSF ended on July 14
...
write.csv(est5, "Y:/Fishery_Monitor/Mark Selective Fishery monitoring/MSF Reference Fisheries/Creel VS RF/Raw data/Prepped Data/Catch_est_CreelVsRef(2023).csv")
