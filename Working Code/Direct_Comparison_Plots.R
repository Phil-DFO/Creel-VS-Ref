#THis code will create plots compring size and mark status between cree and refernce fishery 
#Author: Philip Lemp 
#Date: June 20,2025
...
# Optional: Make Month a factor to control order (e.g., May → June → July)
legal_rate_long$Month <- factor(legal_rate_long$Month, levels = c( "April", "May", "June", "July"))
...
"Compare_Plotting" <- function(Data, YEAR, Stat) {
  ...
  #Set month levels
  month_levels <- c("April", "May", "June", "July")
  
  #Foactor month levels 
  Data <- Data %>%
    mutate(Month = factor(Month, levels = month_levels))
  
  # All combinations of months and MSFs
  full_facet_grid <- expand.grid(
    Month = month_levels,
    MSF = unique(Data$MSF),
    stringsAsFactors = FALSE
  )
  
  # Join with actual data, preserving full grid
  Data_full <- full_facet_grid %>%
    left_join(Data, by = c("Month", "MSF"))
  ...
  # Plot
  Rate_Compare_Plot <- ggplot(Data_full, aes(x = Source, y = Rate, fill = Source)) +
    ...
    facet_grid(Month ~ MSF,
               scales = "free_x", 
               space = "free_x", 
               labeller = labeller(
                 MSF = c(
                   "Gulf_Saanich"  = "Gulf Islands &\nSaanich Inlet", 
                   "Victoria_Haro" = "Vicotira &\nHaro Strait", 
                   "Becher"        = "Becher Bay", 
                   "Sechelt"       = "Sechelt Inlet" 
                 )
               )) +
    labs(
      x = NULL,
    ...
    scale_y_continuous(breaks = seq(0,100, by = 25), 
                       limits = c(0, max(Data_full$UCL, na.rm = TRUE) * 1.2))+
  ...
  # Check which facets are empty
  empty_facets <- Data_full %>%
    group_by(Month, MSF) %>%
    summarise(is_empty = all(is.na(Rate)), .groups = "drop") %>%
    filter(is_empty)
  
  y_start <- max(Data_full$UCL, na.rm = TRUE) * 0.5
  
  x_levels <- unique(Data_full$Source)
  x_start <- length(x_levels) / 2
  
  # Create data frame for empty facets with correct positioning for 'NO DATA'
  empty_labels <- empty_facets %>%
    mutate(
      label = "No data",
      x = x_start,
      y = y_start
  ...
  #Add NA when MSFs were not open
  
  if(YEAR == 2023){
    empty_labels <- empty_labels %>% 
      mutate(label = case_when(
        Month == "April" & MSF %in% c("Victoria_Haro", "Gulf_Saanich") ~ "NA", 
        Month %in% c("June", "July") & MSF %in% c("Victoria_Haro") ~ "NA",
        TRUE ~ label) #preserve existing vlaues 
        )
  }else if(YEAR == 2024){ 
    empty_labels <- empty_labels %>% 
      mutate(label = case_when(
  ...
  Rate_Compare_Plot <- Rate_Compare_Plot + 
    geom_text(
      data = empty_labels,
...
ggsave(Mark_Rate_Compare_2023, file ="Y:/Fishery_Monitor/Mark Selective Fishery monitoring/MSF Reference Fisheries/Creel VS RF/Images/Direct Compare/Mark_Rate_Compare_2023.jpeg", width = 7, height = 6, dpi = 300, units = "in")

Mark_Rate_Compare_2024 <- Compare_Plotting(Data = mark_rate_long, YEAR = 2024, Stat = "Mark_Rate")

ggsave(Mark_Rate_Compare_2024, file ="Y:/Fishery_Monitor/Mark Selective Fishery monitoring/MSF Reference Fisheries/Creel VS RF/Images/Direct Compare/Mark_Rate_Compare_2024.jpeg", width = 7, height = 6, dpi = 300, units = "in")
...
ggsave(Legal_Rate_Compare_2023, file ="Y:/Fishery_Monitor/Mark Selective Fishery monitoring/MSF Reference Fisheries/Creel VS RF/Images/Direct Compare/Legal_Rate_Compare_2023.jpeg", width = 7, height = 6, dpi = 300, units = "in")

Legal_Rate_Compare_2024 <- Compare_Plotting(Data = legal_rate_long, YEAR = 2024, Stat = "Legal_Rate")

ggsave(Legal_Rate_Compare_2024, file ="Y:/Fishery_Monitor/Mark Selective Fishery monitoring/MSF Reference Fisheries/Creel VS RF/Images/Direct Compare/Legal_Rate_Compare_2024.jpeg", width = 7, height = 6, dpi = 300, units = "in")
...
write.csv(Legal_Rate_Dif, file = "Y:/Fishery_Monitor/Mark Selective Fishery monitoring/MSF Reference Fisheries/Creel VS RF/Images/Direct Compare/Legal_Rate_Dif.csv")
...
write.csv(Mark_Rate_Dif, file = "Y:/Fishery_Monitor/Mark Selective Fishery monitoring/MSF Reference Fisheries/Creel VS RF/Images/Direct Compare/Mark_Rate_Dif.csv")
