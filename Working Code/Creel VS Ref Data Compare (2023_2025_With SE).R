#this code will summrize sample size of refernce fisher catch. 
#Suptract it from the release estimtates and the compare the sample sized. 
#Date 10/4/2024
#Author: Phil
...
Ref_Catch_Sum <- Ref_Catch %>%
  group_by(Year, Month, Creel_Sub_Area, Adipose_Fin_Clipped, Size_Class, Disposition) %>%
...
Creel_Catch_clean <- Creel_Catch %>% 
  select(year,month, creelsub, mark, size_class, disposition,  est_sum, se_sum)
...
Creel_Catch_clean <- Creel_Catch_clean %>%
  mutate( Adipose_Fin_Clippped = case_when(
...
Creel_VS_Ref_counts <- Creel_VS_Ref %>%
  filter(Adipose_Fin_Clippped %in% c("Y", "N"), Size_Class == "Legal") %>% 
...
totals <-  Creel_VS_Ref %>% filter(Adipose_Fin_Clippped %in% c("Y", "N"), Size_Class == "Legal") %>%
  group_by(Creel_Sub_Area, Month, Year) %>% 
  summarise(Creel_Total = sum(Creel_Catch_Estimate, na.rm = T), Ref_Total = sum(Referece_Fishery_Catch, na.rm = T))

marked <- Creel_VS_Ref_counts %>% group_by(Creel_Sub_Area, Month, Year) %>% filter(Adipose_Fin_Clippped == "Y") %>% 
  summarise(creel_marked = sum(Creel_Catch_Estimate, na.rm = T),
            ref_marked   = sum(Referece_Fishery_Catch, na.rm = T), 
            SE_Creel_Marked = sqrt(sum(Creel_Catch_SE^2)) )

unmarked <- Creel_VS_Ref_counts %>% group_by(Creel_Sub_Area, Month, Year) %>%
  filter(Adipose_Fin_Clippped == "N") %>% summarise(creel_unmarked = sum(Creel_Catch_Estimate, na.rm = T),
...
Legal_Proportions_Month_Sub <- Legal_Proportions_Month_Sub %>%
  ...
  select(Year, Month, Creel_Sub_Area, Creel_Total, creel_marked, SE_Creel_Marked, 
...
write.csv(Legal_Proportions_Month_Sub, here::here("data", "Legal_Proportions_Month_Sub(2023&2024).csv"), row.names = FALSE)
